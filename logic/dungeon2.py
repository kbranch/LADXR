...
class Dungeon2:
    def __init__(self, options, world_setup, r):
        ...
        dungeon2_l2 = Location("D2 Left Door", dungeon=2).connect(entrance, FOUND(KEY2, 5), id="bw")  # towards map chest
        dungeon2_map_chest = Location(dungeon=2).add(DungeonChest(0x12E)).connect(dungeon2_l2, AND(r.enemy_requirements["KEESE"], OR(FEATHER, HOOKSHOT)), id="bx")  # map chest
        dungeon2_r2 = Location("D2 After Torches", dungeon=2).connect(entrance, r.fire, id="by")
        Location(dungeon=2).add(DroppedKey(0x132)).connect(dungeon2_r2, AND(r.enemy_requirements["STALFOS_EVASIVE"], r.enemy_requirements["STALFOS_AGGRESSIVE"]), id="bz")
        Location(dungeon=2).add(DungeonChest(0x137)).connect(dungeon2_r2, AND(FOUND(KEY2, 5), r.enemy_requirements["MASKED_MIMIC_GORIYA"]), id="c0")  # compass chest
        ...
        if options.logic == "casual":
            shyguy_key_drop = Location(dungeon=2).add(DroppedKey(0x134)).connect(dungeon2_r3, AND(FEATHER, r.enemy_requirements["MASKED_MIMIC_GORIYA"]), id="c4")  # shyguy drop key
        else:
            shyguy_key_drop = Location(dungeon=2).add(DroppedKey(0x134)).connect(dungeon2_r3, OR(r.rear_attack, AND(FEATHER, r.enemy_requirements["MASKED_MIMIC_GORIYA"])), id="c5")  # shyguy drop key
        dungeon2_r5 = Location("D2 Pushblock Room", dungeon=2).connect(dungeon2_r4, FOUND(KEY2, 3), id="c6")  # push two blocks together room with owl statue
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
        ...
        dungeon2_ghosts_room = Location("D2 Boo Buddies Room", dungeon=2).connect(miniboss, FOUND(KEY2, 5), id="cb")
        dungeon2_ghosts_chest = Location(dungeon=2).add(DungeonChest(0x120)).connect(dungeon2_ghosts_room, OR(r.fire, r.enemy_requirements["BOO_BUDDY"]), id="cc")  # bracelet chest
        dungeon2_r6 = Location("D2 After Boo Buddies", dungeon=2).add(DungeonChest(0x122)).connect(miniboss, POWER_BRACELET, id="cd")
        dungeon2_boss_key = Location(dungeon=2).add(DungeonChest(0x127)).connect(dungeon2_r6, AND(r.enemy_requirements["KEESE"], r.enemy_requirements["MOBLIN"], OR(r.enemy_requirements["POLS_VOICE"], POWER_BRACELET)), id="ce")
        dungeon2_pre_stairs_boss = Location("D2 Before Boss Stairs", dungeon=2).connect(dungeon2_r6, AND(r.enemy_requirements["ZOL"], OR(r.enemy_requirements["POLS_VOICE"], POWER_BRACELET), POWER_BRACELET, FOUND(KEY2, 5)), id="cf")
        ...
        if options.logic == 'hell':
            ...
            dungeon2_pre_stairs_boss.connect(dungeon2_r6, AND(r.hookshot_clip_block, r.enemy_requirements["ZOL"], r.enemy_requirements["POLS_VOICE"], FOUND(KEY2, 5)), id="cq") # hookshot clip through the pot using both pol's voice
