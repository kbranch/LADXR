...
class Dungeon6:
    def __init__(self, options, world_setup, r, *, raft_game_chest=True):
        ...
        # right side
        to_miniboss = Location("D6 Before Miniboss", dungeon=6).connect(entrance, FOUND(KEY6, 1), id="hs")
        ...
        center_2_and_upper_right_side = Location(dungeon=6).add(DungeonChest(0x1B1)).connect(center_1, AND(COUNT(POWER_BRACELET, 2), PEGASUS_BOOTS, r.enemy_requirements["POLS_VOICE"], FOUND(KEY6, 2)), id="hz") # top right chest horseheads
        boss_key = Location(dungeon=6).add(DungeonChest(0x1B6))
        center_2_and_upper_right_side.connect(boss_key, AND(HOOKSHOT, POWER_BRACELET, r.miniboss_requirements["DODONGO"], FOUND(KEY6, 3)), one_way=True, id="i0")
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
