...
class Dungeon6:
    def __init__(self, options, world_setup, r, *, raft_game_chest=True):
        ...
        after_a_passage.connect(pre_bracelet_room, r.hit_switch, back=False) # Inside Pegs Before L2 Bracelet Chest --> Outside Pegs Before L2 Bracelet Room #TODO: Consider connect with fake SWITCH_6 item for consciseness, goal for stair shuffle preparedness
        ...
        bracelet_room.connect(entrance, COUNT(POWER_BRACELET, 2), back=False) # L2 Bracelet Room --> Entrance
        ...
        star_area.connect(south_star_area, None, back=False) # Northwest Area --> Before Northwest Area
        ...
        top_left_room.connect(star_area, POWER_BRACELET, back=False) # Northwest Horse Head Room --> Northwest Area
        ...
        miniboss_room.connect(after_miniboss, r.miniboss_requirements[world_setup.miniboss_mapping[5]], back=False) # Miniboss Room --> After Miniboss
        after_miniboss.connect(miniboss_room, COUNT(POWER_BRACELET, 2), back=False) # After Miniboss --> Miniboss Room
        after_miniboss.connect(before_b_passage, COUNT(POWER_BRACELET, 2), back=False) # After Miniboss --> Boss Passageway Revealed
        before_b_passage.connect(after_miniboss, None, back=False) # Boss Passageway Revealed --> After Miniboss
        #TODO: after_miniboss.connect(before_miniboss, None, back=False) # walk north in room north of miniboss to wrap around. NOTE: get stuck in door if you haven't defeated the miniboss, but you can use items to be closer to screen edge, probably oob
        # east
        entrance.connect(four_wizrobe_room, COUNT(POWER_BRACELET, 2), back=False) # Entrance --> Four Wizrobe Room
        four_wizrobe_room.connect(entrance, r.enemy_requirements["WIZROBE"], back=False) # Four Wizrobe Room --> Entrance
        four_wizrobe_room.connect(blade_trap_room, r.enemy_requirements["WIZROBE"], back=False) # Four Wizrobe Room --> Blade Trap Room
        blade_trap_room.connect(after_blade_trap, AND(FEATHER, r.hit_switch), back=False) #  --> Ledge After Blade Trap #TODO: Consider connect with fake SWITCH_6 item for consciseness, goal for stair shuffle preparedness
        after_blade_trap.connect(after_blade_trap_chest8, None) # Ledge After Blade Trap <--> Four Wizrobe Ledge Chest
        after_blade_trap.connect(four_wizrobe_room, None, back=False) # Ledge After Blade Trap --> Four Wizrobe Room
        ...
        dodongo_room.connect(after_c_passage, OR(FEATHER, r.miniboss_requirements["DODONGO"]), back=False) # Dodongo Room --> Pols Room
        after_c_passage.connect(spark_pot_maze, r.enemy_requirements["POLS_VOICE"]) # Pols Room <--> Spark & Pot Maze
        after_c_passage.connect(dodongo_room, r.enemy_requirements["POLS_VOICE"], back=False) # Pols Room --> Dodongo Room
        ...
        waterway_east_ledge.connect(dodongo_room, None, back=False) # Outside Dodongo Room --> Dodongo Room
        waterway_east_ledge.connect(waterway, None, back=False) # Outside Dodongo Room --> Waterway
        waterway_east_ledge.connect(hookshot_block, HOOKSHOT, back=False) # Outside Dodongo Room --> Waterway Hookshot Block
        ...
        before_c_passage.connect(flying_bomb_room, POWER_BRACELET, back=False) # Stairs West of Horsehead Hallway --> Flying Bomb Room
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            before_b_passage.connect(after_b_passage, r.boots_dash_2d, back=False) # boots dash over 1 block gaps in sidescroller
            after_b_passage.connect(before_b_passage, AND(r.boots_dash_2d, r.boots_bonk), back=False) # boots dash over 1 block gaps in sidescroller, then bonk to get on ladder
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            wizrobe_switch_room.connect(south_star_area, r.super_jump_feather, back=False) # path from entrance to left_side: use superjumps to pass raised blocks
            ...
            flying_bomb_room.connect(after_b_passage, r.shaq_jump, back=False) # face left and shaq jump off keyblock to get into floating tile room
        ...
        if options.logic == 'hell':
            ...
            entrance.connect(south_star_area, AND(POWER_BRACELET, r.stun_mask_mimic, r.throw_enemy), back=False) #TODO: REMOVE and replace with below
            #TODO: entrance.connect(south_star_area, AND(POWER_BRACELET, OR(r.boots_superhop, AND(r.stun_mask_mimic, r.throw_enemy), SWORD)), back=False) # very difficult ways to hit the switch while making it over the peg - stun and throw the mask mimic up at the wall and run, or spin atack whilc walking to the peg while wall clipped, or normal boots superhop from NW corner of room
