...
class Dungeon4:
    def __init__(self, options, world_setup, r):
        ...
        double_locked_room = Location("D4 Double Locked Room", dungeon=4).connect(right_of_entrance, FOUND(KEY4, 5), one_way=True, id="ej")
        right_of_entrance.connect(double_locked_room, FOUND(KEY4, 1), one_way=True, id="ek")
        after_double_lock = Location("D4 Crossroads South", dungeon=4).connect(double_locked_room, AND(FOUND(KEY4, 4), OR(FEATHER, FLIPPERS)), one_way=True, id="el")
        double_locked_room.connect(after_double_lock, AND(FOUND(KEY4, 2), OR(FEATHER, FLIPPERS)), one_way=True, id="em")
        ...
        before_miniboss = Location("D4 Before Miniboss", dungeon=4).connect(north_crossroads, FOUND(KEY4, 3), id="ep")
        ...
        miniboss_room = Location("D4 Miniboss Room", dungeon=4).connect(before_miniboss, FOUND(KEY4, 5), id="eu")
        ...
        before_boss = Location("D4 Before Boss", dungeon=4).connect(before_miniboss, AND(r.attack_hookshot, FLIPPERS, FOUND(KEY4, 5)), id="f0")
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            before_boss.connect(left_water_area, r.tight_jump, id="fd") # jump from bottom right corner of boss door room
