...
class Dungeon3:
    def __init__(self, options, world_setup, r):
        ...
        Location(dungeon=3).add(DungeonChest(0x151)).connect(area2, AND(r.enemy_requirements["GEL"], r.enemy_requirements["MOBLIN_SWORD"], r.enemy_requirements["BOUNCING_BOMBITE"]), id="cx")  # First chest with key
        area2.add(DungeonChest(0x14F))  # Second chest with slime
        area3 = Location("D3 After Zol Chest", dungeon=3).connect(area2, OR(r.enemy_requirements["HIDING_ZOL"], PEGASUS_BOOTS), id="cy") # need to kill slimes to continue or pass through left path
        ...
        dungeon3_miniboss_room = Location("D3 Miniboss Room", dungeon=3).connect(area_right, AND(r.enemy_requirements["ZOL"], r.enemy_requirements["GEL"]), id="dc")
        ...
        compass_chest = Location(dungeon=3).add(DungeonChest(0x142)).connect(area_right, AND(r.enemy_requirements["HIDING_ZOL"], r.enemy_requirements["TIMER_BOMBITE"]), id="de")
        dungeon3_3_bombite_room = Location(dungeon=3).add(DroppedKey(0x141)).connect(compass_chest, AND(r.enemy_requirements["BOUNCING_BOMBITE"], BOMB), id="df") # 3 bombite room
        Location(dungeon=3).add(DroppedKey(0x148)).connect(area_right, AND(r.enemy_requirements["HIDING_ZOL"], r.enemy_requirements["PAIRODD"]), id="dg") # 2 zol 2 owl drop key
        Location(dungeon=3).add(DungeonChest(0x144)).connect(area_right, AND(r.enemy_requirements["STALFOS_AGGRESSIVE"], r.enemy_requirements["ZOL"]), id="dh")  # map chest
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            area_up.connect(dungeon3_raised_blocks_east, AND(r.throw_pot, r.enemy_requirements["HIDING_ZOL"]), one_way=True, id="dx") # use pots to hit the switch

        if options.logic == 'glitched' or options.logic == 'hell':
            area2.connect(dungeon3_raised_blocks_east, AND(r.enemy_requirements["HIDING_ZOL"], r.super_jump_feather), one_way=True, id="dy") # use superjump to get over the bottom left block
            ...
            area3.connect(dungeon3_zol_stalfos, AND(r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"], r.hookshot_clip_block), one_way=True, id="e0") # hookshot clip through the northern push block next to raised blocks chest to get to the zol
        ...
        if options.logic == 'hell':
            area2.connect(dungeon3_raised_blocks_east, AND(r.enemy_requirements["HIDING_ZOL"], r.boots_superhop), one_way=True, id="e3") # use boots superhop to get over the bottom left block
            ...
            area_up.connect(dungeon3_zol_stalfos, AND(r.super_jump_feather, r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"]), one_way=True, id="e5") # use superjump near top blocks chest to get to zol without boots, keep wall clip on right wall to get a clip on left wall or use obstacles
            ...
            compass_chest.connect(dungeon3_3_bombite_room, AND(r.enemy_requirements["BOUNCING_BOMBITE"], OR(BOW, MAGIC_ROD, AND(OR(FEATHER, PEGASUS_BOOTS), OR(SWORD, MAGIC_POWDER)))), one_way=True, id="ea") # 3 bombite room from the left side, use a bombite to blow open the wall without bombs
            pre_boss.connect(towards_boss4, AND(r.enemy_requirements["PAIRODD"], FEATHER, POWER_BRACELET), id="eb") # use bracelet super bounce glitch to pass through first part underground section
            pre_boss.connect(towards_boss4, AND(r.enemy_requirements["PAIRODD"], r.boots_bonk_2d_spikepit), id="ec") # use medicine invulnerability to pass through the 2d section with a boots bonk to reach the staircase
